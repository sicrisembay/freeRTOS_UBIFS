...
static const char *partition_default = PARTITION_DEFAULT;
...
static int part_validate_eraseblock(struct mtdids *id, struct part_info *part)
{
	...
	if (!mtd->numeraseregions) {
...
#endif /* __ZPL_BUILD__ */
			debug("%s%d: partition (%s) start offset"
			       "alignment incorrect\n",
...
#endif /* __ZPL_BUILD__ */
			debug("%s%d: partition (%s) size alignment incorrect\n",
			       MTD_DEV_TYPE(id->type), id->num, part->name);
			return 1;
		}
	} else {
		...
		debug("%s%d: partition (%s) start offset alignment incorrect\n",
		       MTD_DEV_TYPE(id->type), id->num, part->name);
		return 1;
...
static int part_validate(struct mtdids *id, struct part_info *part)
{
	...
	if (part->offset > id->size) {
		debug("%s: offset %08x beyond flash size %08x\n",
				id->mtd_id, part->offset, id->size);
		return 1;
	}

	if ((part->offset + part->size) <= part->offset) {
		debug("%s%d: partition (%s) size too big\n",
				MTD_DEV_TYPE(id->type), id->num, part->name);
		return 1;
	}

	if (part->offset + part->size > id->size) {
		debug("%s: partitioning exceeds flash size\n", id->mtd_id);
		return 1;
	}
...
static int part_del(struct mtd_device *dev, struct part_info *part)
{
	...
	if (dev == current_mtd_dev) {
		...
		if (curr_pi) {
			if (curr_pi == part) {
				debug("current partition deleted, resetting current to 0\n");
...
static int part_sort_add(struct mtd_device *dev, struct part_info *part)
{
	...
	list_for_each(entry, &dev->parts) {
		...
		if ((new_pi->offset == pi->offset) && (pi->offset == 0)) {
			debug("cannot add second partition at offset 0\n");
			return 1;
		}
	...
	*ret = NULL;
	...
	if (*p == '-') {
		...
		if (size < MIN_PART_SIZE) {
			debug("partition size too small (%x)\n", size);
			return 1;
		}
	}
	...
	if (*p == '(') {
		...
		if ((p = strchr(name, ')')) == NULL) {
			debug("no closing ) found in partition name\n");
			return 1;
		}
		name_len = p - name + 1;
		if ((name_len - 1) == 0) {
			debug("empty partition name\n");
			return 1;
		}
		p++;
	} else {
	...
	if (*p == ',') {
		if (size == SIZE_REMAINING) {
			*ret = NULL;
			debug("no partitions allowed after a fill-up partition\n");
			return 1;
		}
		*ret = ++p;
	} else if ((*p == ';') || (*p == '\0')) {
		*ret = p;
		...
		debug("unexpected character '%c' at the end of partition\n", *p);
		*ret = NULL;
		return 1;
	}
	...
	if (name) {
		...
		sprintf(part->name, "0x%08x@0x%08x", size, offset);
		part->auto_name = 1;
	}
...
static int device_parse(const char *const mtd_dev, const char **ret, struct mtd_device **retdev)
{
	...
	*retdev = NULL;
	...
	if (num_parts == 0) {
		debug("no partitions for device %s%d (%s)\n",
	...
	if (p) {
		if (*p == ';') {
			...
			debug("unexpected character '%c' at the end of device\n", *p);
			if (ret)
				*ret = NULL;
			return 1;
		}
...
static void print_partition_table(void)
{
	...
	list_for_each(dentry, &devices) {
		...
		debug(" #: name\t\tsize\t\tnet size\toffset\t\tmask_flags\n");
		...
		list_for_each(pentry, &dev->parts) {
			...
			debug("%2d: %-20s0x%08x\t0x%08x%s\t0x%08x\t%d\n",
		...
		debug("\ndevice %s%d <%s>, # parts = %d\n",
				MTD_DEV_TYPE(dev->id->type), dev->id->num,
				dev->id->mtd_id, dev->num_parts);
		debug(" #: name\t\tsize\t\toffset\t\tmask_flags\n");
...
static void list_partitions(void)
{
	...
	if (current_mtd_dev) {
		...
		if (part) {
			...
			debug("could not get current partition info\n\n");
		}
	}
	...
	/*
	 * Using debug() here results in printbuffer overflow
		...
		u8 *part_num, struct part_info **part)
{
	...
	*part_num = 0;
	...
	if ((*p++ != ',') || (*p == '\0')) {
		debug("no partition number specified\n");
		return 1;
	}
	pnum = simple_strtoul(p, (char **)&p, 0);
	if (*p != '\0') {
		debug("unexpected trailing character '%c'\n", *p);
		return 1;
	}

	if ((*dev = device_find(type, dnum)) == NULL) {
		debug("no such device %s%d\n", MTD_DEV_TYPE(type), dnum);
		return 1;
	}

	if ((*part = mtd_part_info(*dev, pnum)) == NULL) {
		debug("no such partition\n");
		*dev = NULL;
		return 1;
	}
...
static int delete_partition(const char *id)
{
	...
	if (find_dev_and_part(id, &dev, &pnum, &part) == 0) {
		...
		if (generate_mtdparts_save(last_parts, MTDPARTS_MAXLEN) != 0) {
			debug("generated mtdparts too long, resetting to null\n");
			return 1;
		}
		return 0;
	}

	debug("partition %s not found\n", id);
	return 1;
}
			     ...
			     uint64_t *next_offset)
{
	...
	if (truncated) {
		debug("truncated partition %s to %ld bytes\n", part->name,
...
static int parse_mtdparts(const char *const mtdparts)
{
	...
	if (mtd_devices_init() != 0) {
		debug("could not initialise device list\n");
		return err;
	}
	...
	if (strncmp(p, "mtdparts=", 9) != 0) {
		debug("mtdparts variable doesn't start with 'mtdparts='\n");
		return err;
	}
	p += 9;
	...
	while (*p != '\0') {
		...
		if (device_find(dev->id->type, dev->id->num) != NULL) {
			debug("device %s%d redefined, please correct mtdparts variable\n",
					MTD_DEV_TYPE(dev->id->type), dev->id->num);
			break;
		}
...
static int parse_mtdids(const char *const ids)
{
	...
	while(p && (*p != '\0')) {
		...
		if (*p != '=') {
			debug("mtdids: incorrect <dev-num>\n");
			break;
		}
		p++;
		...
		if (mtd_id_len == 0) {
			debug("mtdids: no <mtd-id> identifier\n");
			break;
		}
		...
		if (double_entry) {
			debug("device id %s%d redefined, please correct mtdids variable\n",
...
int mtdparts_init(void)
{
...
#if defined(CONFIG_SYS_MTDPARTS_RUNTIME)
		board_mtdparts_default(&, &mtdparts_default);
#endif
		use_defaults = 1;
		initialized = 1;
	}
	...
	if (!ids) {
		if (mtdids_default) {
			...
			debug("mtdids not defined, no default present\n");
			return 1;
		}
	}
	if (strlen(ids) > MTDIDS_MAXLEN - 1) {
		debug("mtdids too long (> %d)\n", MTDIDS_MAXLEN);
		return 1;
	}
	...
	if (!parts) {
		if (mtdparts_default && use_defaults) {
			...
			debug("mtdparts variable not set, see 'help mtdparts'\n");
	}

	if (parts && (strlen(parts) > MTDPARTS_MAXLEN - 1)) {
		debug("mtdparts too long (> %d)\n", MTDPARTS_MAXLEN);
		return 1;
	}
	...
	if (parts && ((last_parts[0] == '\0') || ((strcmp(last_parts, parts) != 0)) || ids_changed)) {
		...
		if (list_empty(&devices)) {
			debug("mtdparts_init: no valid partitions\n");
			return 1;
		}
...
static struct part_info* mtd_part_info(struct mtd_device *dev, unsigned int part_num)
{
	...
	if (part_num >= dev->num_parts) {
		debug("invalid partition number %d for device %s%d (%s)\n",
				part_num, MTD_DEV_TYPE(dev->id->type),
				dev->id->num, dev->id->mtd_id);
		return NULL;
	}
...
/* command line only */
	...
	if (argc < 2) {
		debug("no partition id specified\n");
		return 1;
	}
	...
	debug("partition changed to %s%d,%d\n",
			MTD_DEV_TYPE(dev->id->type), dev->id->num, pnum);
...
#define PART_ADD_DESC_MAXLEN 64
		...
		if ((id = id_find(type, num)) == NULL) {
			debug("no such device %s defined in mtdids variable\n", argv[2]);
			return 1;
		}
		...
		if (len >= PART_ADD_DESC_MAXLEN) {
			debug("too long partition description\n");
			return 1;
		}
