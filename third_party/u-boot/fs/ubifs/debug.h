...
#define UBIFS_DEBUG_VIA_SEMIHOST_PRINTF     (0)
...
#define ubifs_assert(expr) do {                                                \
	if (unlikely(!(expr))) {                                               \
		debug("UBIFS assert failed in %s at %u\n",                   \
		       __func__, __LINE__);                                    \
		dump_stack();                                                  \
	}                                                                      \
} while (0)
...
#define ubifs_assert_cmt_locked(c) do {                                        \
	if (unlikely(down_write_trylock(&(c)->commit_sem))) {                  \
		up_write(&(c)->commit_sem);                                    \
		debug("commit lock is not locked!\n");                       \
		ubifs_assert(0);                                               \
	}                                                                      \
} while (0)

#if(UBIFS_DEBUG_VIA_SEMIHOST_PRINTF == 1)
#define ubifs_dbg_msg(type, fmt, ...) \
    printf("UBIFS DBG " type ": " fmt "\n",                              \
         ##__VA_ARGS__)
#else
#define ubifs_dbg_msg(type, fmt, ...) \
	pr_debug("UBIFS DBG " type ": " fmt "\n",                              \
		 ##__VA_ARGS__)
#endif /* #if(UBIFS_DEBUG_VIA_SEMIHOST_PRINTF == 1) */
