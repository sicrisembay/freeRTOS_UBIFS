...
static int validate_inode(struct ubifs_info *c, const struct inode *inode)
{
	...
	if (inode->i_size > c->max_inode_sz) {
		ubifs_err(c, "inode is too large (%ld)",
...
static int ubifs_statfs(struct dentry *dentry, struct kstatfs *buf)
{
	...
	dbg_gen("free space %ld bytes (%ld blocks)",
...
#else
	c->max_bud_cnt = tmp64 / c->leb_size;
#endif /* __ZPL_BUILD__ */
...
static int mount_ubifs(struct ubifs_info *c)
{
	...
	ubifs_msg(c, "LEB size: %d bytes (%d KiB), min./max. I/O unit sizes: %d bytes/%d bytes",
		  ...
		  c->max_write_size);
	ubifs_msg(c, "FS size: %ld bytes (%ld MiB, %d LEBs), journal size %ld bytes (%ld MiB, %d LEBs)",
		  x, x >> 20, c->main_lebs,
		  y, y >> 20, c->log_lebs + c->max_bud_cnt);
	ubifs_msg(c, "reserved for root: %lu bytes (%lu KiB)",
	...
	dbg_gen("total index bytes:   %ld (%ld KiB, %ld MiB)",
	...
	dbg_gen("node sizes:          data %d, inode %d, dentry %d",
		UBIFS_DATA_NODE_SZ, UBIFS_INO_NODE_SZ, UBIFS_DENT_NODE_SZ);
	dbg_gen("node sizes:          trun %d, sb %d, master %d",
		UBIFS_TRUN_NODE_SZ, UBIFS_SB_NODE_SZ, UBIFS_MST_NODE_SZ);
	dbg_gen("node sizes:          ref %d, cmt. start %d, orph %d",
		UBIFS_REF_NODE_SZ, UBIFS_CS_NODE_SZ, UBIFS_ORPH_NODE_SZ);
	dbg_gen("max. node sizes:     data %d, inode %d dentry %d, idx %d",
	...
	dbg_gen("max. dark space:     %d (%d KiB, %d MiB)",
		x, x >> 10, x >> 20);
	dbg_gen("maximum bud bytes:   %d (%d KiB, %d MiB)",
		c->max_bud_bytes, c->max_bud_bytes >> 10,
		c->max_bud_bytes >> 20);
	dbg_gen("BG commit bud bytes: %d (%d KiB, %d MiB)",
		c->bg_bud_bytes, c->bg_bud_bytes >> 10,
		c->bg_bud_bytes >> 20);
	dbg_gen("current bud bytes    %d (%ld KiB, %ld MiB)",
		c->bud_bytes, c->bud_bytes >> 10, c->bud_bytes >> 20);
	dbg_gen("max. seq. number:    %lu", c->max_sqnum);
	dbg_gen("commit number:       %lu", c->cmt_no);
