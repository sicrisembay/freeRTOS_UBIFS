...
void ubifs_dump_node(const struct ubifs_info *c, const void *node)
{
	...
	case UBIFS_MST_NODE:
	{
		...
		pr_err("\thighest_inum   %lu\n",
		       (unsigned long long)le64_to_cpu(mst->highest_inum));
		pr_err("\tcommit number  %lu\n",
		...
		pr_err("\ttotal_free     %lu\n",
		       (unsigned long long)le64_to_cpu(mst->total_free));
		pr_err("\ttotal_dirty    %lu\n",
		       (unsigned long long)le64_to_cpu(mst->total_dirty));
		pr_err("\ttotal_used     %lu\n",
		       (unsigned long long)le64_to_cpu(mst->total_used));
		pr_err("\ttotal_dead     %lu\n",
		       (unsigned long long)le64_to_cpu(mst->total_dead));
		pr_err("\ttotal_dark     %lu\n",
...
void ubifs_dump_lstats(const struct ubifs_lp_stats *lst)
{
	...
	pr_err("\ttaken_empty_lebs %d, total_free %ld, total_dirty %ld\n",
	       lst->taken_empty_lebs, lst->total_free, lst->total_dirty);
	pr_err("\ttotal_used %ld, total_dark %ld, total_dead %ld\n",
...
void ubifs_dump_budg(struct ubifs_info *c, const struct ubifs_budg_info *bi)
{
	...
	spin_lock(&dbg_lock);
	pr_err("(pid %d) Budgeting info: data budget sum %ld, total budget sum %ld\n",
	       current->pid, bi->data_growth + bi->dd_growth,
	       bi->data_growth + bi->dd_growth + bi->idx_growth);
	pr_err("\tbudg_data_growth %ld, budg_dd_growth %ld, budg_idx_growth %ld\n",
	       bi->data_growth, bi->dd_growth, bi->idx_growth);
	pr_err("\tmin_idx_lebs %d, old_idx_sz %lu, uncommitted_idx %ld\n",
	...
	pr_err("\tfreeable_cnt %d, calc_idx_sz %ld, idx_gc_cnt %d\n",
	...
	pr_err("\tavailable: %ld, outstanding %ld, free %ld\n",
...
int dbg_check_space_info(struct ubifs_info *c)
{
	...
	if (free != d->saved_free) {
		ubifs_err(c, "free space changed from %ld to %ld",
			  d->saved_free, free);
		goto out;
	}
...
int dbg_check_synced_i_size(const struct ubifs_info *c, struct inode *inode)
{
	...
	if (ui->ui_size != ui->synced_i_size && !ui->dirty) {
		ubifs_err(c, "ui_size is %ld, synced_i_size is %ld, but inode is clean",
			  ui->ui_size, ui->synced_i_size);
		ubifs_err(c, "i_ino %lu, i_mode %#x, i_size %ld", inode->i_ino,
...
int dbg_check_dir(struct ubifs_info *c, const struct inode *dir)
{
	...
	if (i_size_read(dir) != size) {
		ubifs_err(c, "directory inode %lu has size %lu, but calculated size is %lu",
...
int dbg_check_idx_size(struct ubifs_info *c, long long idx_size)
{
	...
	if (calc != idx_size) {
		ubifs_err(c, "index size check failed: calculated size is %ld, should be %ld",
			  calc, idx_size);
		dump_stack();
		return -EINVAL;
	}
		      ...
		      void *priv)
{
	...
	if (le64_to_cpu(ch->sqnum) > c->max_sqnum) {
		ubifs_err(c, "too high sequence number, max. is %lu",
			  c->max_sqnum);
		err = -EINVAL;
		goto out_dump;
	}
	...
	if (type == UBIFS_DATA_KEY) {
		...
		if (blk_offs > fscki->size) {
			ubifs_err(c, "data node at LEB %d:%d is not within inode size %ld",
	...
	while (this) {
		...
		if (S_ISDIR(fscki->mode)) {
			...
			if (fscki->calc_sz != fscki->size) {
				ubifs_err(c, "directory inode %lu size is %ld, but calculated size is %ld",
					  (unsigned long)fscki->inum,
		...
		if (fscki->xattr_sz != fscki->calc_xsz) {
			ubifs_err(c, "inode %lu has xattr size %u, but calculated size is %ld",
				  (unsigned long)fscki->inum, fscki->xattr_sz,
				  fscki->calc_xsz);
			goto out_dump;
		}
		if (fscki->xattr_cnt != fscki->calc_xcnt) {
			ubifs_err(c, "inode %lu has %u xattrs, but calculated count is %ld",
				  (unsigned long)fscki->inum,
				  fscki->xattr_cnt, fscki->calc_xcnt);
			goto out_dump;
		}
		if (fscki->xattr_nms != fscki->calc_xnms) {
			ubifs_err(c, "inode %lu has xattr names' size %u, but calculated names' size is %ld",
