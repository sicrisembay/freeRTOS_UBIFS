...
static int apply_replay_entry(struct ubifs_info *c, struct replay_entry *r)
{
	...
	dbg_mntk(&r->key, "LEB %d:%d len %d deletion %d sqnum %lu key ",
...
static int replay_log_leb(struct ubifs_info *c, int lnum, int offs, void *sbuf)
{
	...
	if (c->cs_sqnum == 0) {
		...
		if (le64_to_cpu(node->cmt_no) != c->cmt_no) {
			ubifs_err(c, "first CS node at LEB %d:%d has wrong commit number %lu expected %lu",
				  lnum, offs,
		...
		dbg_mnt("commit start sqnum %lu", c->cs_sqnum);
	}
	...
	list_for_each_entry(snod, &sleb->nodes, list) {
		...
		if (snod->sqnum < c->cs_sqnum) {
			ubifs_err(c, "bad sqnum %lu, commit sqnum %lu",
...
int ubifs_replay_journal(struct ubifs_info *c)
{
	...
	dbg_mnt("finished, log head LEB %d:%d, max_sqnum %lu, highest_inum %lu",
