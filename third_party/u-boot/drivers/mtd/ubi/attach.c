		  ...
		  int ec, const struct ubi_vid_hdr *vid_hdr, int bitflips)
{
	...
	dbg_bld("PEB %d, LEB %d:%d, EC %d, sqnum %lu, bitflips %d",
	...
	while (*p) {
		...
		dbg_bld("this LEB already exists: PEB %d, sqnum %lu, EC %d",
		...
		if (aeb->sqnum == sqnum && sqnum != 0) {
			ubi_err(ubi, "two LEBs with same sequence number %lu",
				sqnum);
			ubi_dump_aeb(aeb, 0);
			ubi_dump_vid_hdr(vid_hdr);
			return -EINVAL;
...
#else
		ai->mean_ec = ai->ec_sum / ai->ec_count;
#endif /* __ZPL_BUILD__ */
	}
...
int ubi_attach(struct ubi_device *ubi, int force_scan)
{
	...
	dbg_gen("max. sequence number:       %d", ai->max_sqnum);
	...
	ubi_rb_for_each_entry(rb1, av, &ai->volumes, rb) {
		...
		ubi_rb_for_each_entry(rb2, aeb, &av->root, u.rb) {
			...
			if (aeb->sqnum != be64_to_cpu(vidh->sqnum)) {
				ubi_err(ubi, "bad sqnum %lu", aeb->sqnum);
				goto bad_vid_hdr;
			}
